...
async function bootstrap() {
  ...
  try {
    ...
    const table = await queryRunner.getTable('perimeters');
    ...
    if (!existingColumns.includes('street')) {
      await queryRunner.query(`ALTER TABLE perimeters ADD COLUMN street TEXT`);
    ...
    if (!existingColumns.includes('postalCode')) {
      await queryRunner.query(`ALTER TABLE perimeters ADD COLUMN postalCode TEXT`);
    ...
    if (!existingColumns.includes('city')) {
      await queryRunner.query(`ALTER TABLE perimeters ADD COLUMN city TEXT`);
    ...
    if (!existingColumns.includes('country')) {
      await queryRunner.query(`ALTER TABLE perimeters ADD COLUMN country TEXT`);
