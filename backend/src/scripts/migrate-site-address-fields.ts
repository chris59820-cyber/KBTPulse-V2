...
async function bootstrap() {
  ...
  try {
    ...
    if (!existingColumns.includes('street')) {
      await queryRunner.query(`ALTER TABLE sites ADD COLUMN street TEXT`);
    ...
    if (!existingColumns.includes('postalCode')) {
      await queryRunner.query(`ALTER TABLE sites ADD COLUMN postalCode TEXT`);
    ...
    if (!existingColumns.includes('city')) {
      await queryRunner.query(`ALTER TABLE sites ADD COLUMN city TEXT`);
    ...
    if (!existingColumns.includes('country')) {
      await queryRunner.query(`ALTER TABLE sites ADD COLUMN country TEXT`);
